...
import { DIRECTIONS, actionToDirection, reverseAction } from './types';
...
describe('direction helpers', () => {
  ...
  test('reverseAction flips each action to its true geometric opposite', () => {
    expect(reverseAction(0)).toBe(1); // up   -> down
    expect(reverseAction(1)).toBe(0); // down -> up
    expect(reverseAction(2)).toBe(3); // left -> right
    expect(reverseAction(3)).toBe(2); // right-> left
  });

  test('reverseAction is an involution over the action space', () => {
    for (const a of [0, 1, 2, 3]) {
      expect(reverseAction(reverseAction(a))).toBe(a);
    }
  });

  test('actionToDirection maps in-range actions', () => {
    expect(actionToDirection(0)).toBe('up');
    expect(actionToDirection(1)).toBe('down');
    expect(actionToDirection(2)).toBe('left');
    expect(actionToDirection(3)).toBe('right');
  });

  test('actionToDirection clamps out-of-range actions (H10 guard)', () => {
    expect(actionToDirection(-2)).toBe('up');
    expect(actionToDirection(-1)).toBe('up');
    expect(actionToDirection(4)).toBe('right');
    expect(actionToDirection(99)).toBe('right');
  });
});
