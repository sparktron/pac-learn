...
export class QLearningAgent {
  ...
  load(data: SerializedPolicy, currentNumGhosts?: number): void {
    ...
    this.q.clear();
    // v7 key string format: "v7:wallMask:pelletDir:gc0:gc1:lastAction:pelletsBucket:powerBucket"
    for (const [keyStr, values] of Object.entries(data.qTable)) {
      const parts = keyStr.split(':');
      if (parts[0] !== 'v7' || parts.length !== 8) continue;
      ...
      const powerBucket   = parseInt(parts[7], 10); // 0-2
