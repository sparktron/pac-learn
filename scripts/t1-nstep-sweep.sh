#!/usr/bin/env bash
# T1: fixed-budget n-step credit-assignment screen for the promoted T2/T7
# linear baseline. This is a screen only; promote a winner only after the
# five-seed confirmation gates in ROADMAP.md.
set -euo pipefail
...
for index in "${!N_STEPS[@]}"; do
  n_step="${N_STEPS[$index]}"
  cell_id=$(printf '%02d' "$index")
  cell_dir="$OUT_BASE/cell-$cell_id"
  printf '%s\t%s\n' "$cell_id" "$n_step" >> "$OUT_BASE/matrix.tsv"
  "$VITE_NODE" scripts/overnight-bench.ts -- \
    "outDir=$cell_dir" algorithm=linear ghosts=2 "seed=$SEED" "episodes=$EPISODES" \
    endgameCurriculum=0.90 stepPenalty=-0.02 alpha=0.02 gamma=0.997 \
    targetSyncSteps=2000 deathPenalty=-50 pelletEscalationMax=10 "nStep=$n_step" \
    "evalEvery=$EPISODES" "evalEpisodes=$EVAL_EPISODES" \
    evalPanels=1000000,2000000,3000000,4000000 snapshotEvery=0 reportEvery=0 \
    > "$cell_dir.log" 2>&1 &
  PIDS+=("$!")
  echo "[start] cell=$cell_id nStep=$n_step"
  if [[ ${#PIDS[@]} -ge $MAX_PARALLEL ]]; then wait_batch; fi
done
wait_batch

if [[ "$failed" -gt 0 ]]; then
  echo "[abort] $failed T1 cell(s) failed; inspect $OUT_BASE/cell-*.log" >&2
  exit 1
fi

REPORT="$OUT_BASE/report.tsv"
printf 'cell\tnStep\tepisodes\ttrainWins\ttrainWinRate\tmeanEvalWinRate\tworstPanelWinRate\tmeanPlP5\tmaxPlP5\n' > "$REPORT"
while IFS=$'\t' read -r cell_id n_step; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$cell_id" "$n_step" "$episodes" "$train_wins" "$train_rate" "$mean_wr" "$worst_wr" "$mean_p5" "$max_p5" >> "$REPORT"
done < "$OUT_BASE/matrix.tsv"

{
  echo "T1 n-step screen"
  echo "seed=$SEED episodes=$EPISODES evalEpisodes=$EVAL_EPISODES cells=${#N_STEPS[@]}"
  echo ""
  echo "Sorted by mean eval win rate, then worst panel (higher is better):"
  head -n 1 "$REPORT"
  tail -n +2 "$REPORT" | sort -t $'\t' -k6,6gr -k7,7gr
  echo ""
